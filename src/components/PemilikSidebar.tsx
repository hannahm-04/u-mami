...
const navItems = [
  { label: "Laporan Keuangan", path: "/pemilik/laporan-keuangan" },
  { label: "Menu Terlaris", path: "/pemilik/menu-terlaris" },
  { label: "Kelola Menu", path: "/pemilik/kelola-menu" },
];

export default function PemilikSidebar() {
  const pathname = usePathname();

  return (
    <div className="w-64 bg-[#8CB9F1] h-screen flex flex-col items-center py-8 shadow-md relative z-10 flex-shrink-0">
      {/* Logo */}
      ...
      <nav className="w-full flex-1 flex flex-col">
        {navItems.map((item) => {
          const isActive = pathname.startsWith(item.path) || (pathname === "/pemilik" && item.path === "/pemilik/laporan-keuangan");
          ...
          return (
            <Link
              ...
              className={`w-full py-4 text-center font-bold text-white transition-colors duration-200 ${
                isActive ? "bg-[#3A7AD5]" : "hover:bg-[#6FA6EB]"
              }`}
            >
              {item.label}
            </Link>
